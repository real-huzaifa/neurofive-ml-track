...
```
steam-reception-predictor/
├── notebooks/
│   └── steam_reception_analysis.ipynb    # Full workflow: clean → EDA → model → export
├── app/
│   ├── app.py                            # Streamlit app
│   ├── requirements.txt
│   └── steam_model.joblib                # Both trained models + metadata (1.1 MB)
├── case_study.md                         # Business writeup
└── README.md
```
...
Download `games.csv` from the [Kaggle dataset](https://www.kaggle.com/datasets/fronkongames/steam-games-dataset)
and place it beside the notebook, then run top to bottom. The final cell exports
`steam_model.joblib`.
...
Push to GitHub, then on [share.streamlit.io](https://share.streamlit.io) point the main file
path at `app/app.py`.

> **Pin your scikit-learn version.** Pickle stores references to internal module paths that
> scikit-learn moves between releases; a mismatch fails at load time with errors like
> `ModuleNotFoundError: No module named '_loss'`. The version in `requirements.txt` must match
> the version that trained the model. Versions before 1.7.2 have no Python 3.14 wheels, which
> is what Streamlit Cloud currently runs.
