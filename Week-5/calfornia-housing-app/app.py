...
Streamlit app serving a gradient boosting regressor trained on the California Housing dataset.
"""
...
# ----------------------------------------------------------------------------
MODEL_PATH = Path(__file__).parent / "housing_model.joblib"
...
try:
    ...
    st.error(
        ...
        "Make sure `housing_model.joblib` sits next to `app.py` in the repo."
    )
    st.stop()
...
st.markdown(
    ...
    "gradient boosting model trained on the 1990 California Housing dataset."
)
...
with st.expander("About this model"):
    st.markdown(
        f"""
**Model:** HistGradientBoostingRegressor (scikit-learn, 300 iterations), trained on the
California Housing dataset (20,640 census block groups, 8 features).
...
Selected after comparing Linear Regression, Random Forest, and XGBoost. XGBoost scored
highest, but its ~58 MB package exceeds the free hosting tier's memory limit, so this
deployment uses scikit-learn's equivalent gradient boosting implementation, which performs
at least as well here with no extra dependency.
